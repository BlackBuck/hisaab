...
export async function fetchMonthlyExpenses(): Promise<MonthlyExpense[]>{
  try {
    const data = await sql<MonthlyExpense[]>`
      SELECT MONTHNAME(date) as month, SUM(amount) as expenses
      FROM expenses
    ...
    console.error("Database error: ", err);
    throw new Error("Database Error: Failed to fetch Monthly Expenses.");
  }
}